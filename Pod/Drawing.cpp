...
void Drawing::draw(LPDIRECT3DTEXTURE9 texture, RECT _srcRect, D3DXVECTOR3 _position, bool _isFlip, D3DCOLOR _color)
{
	if (_isFlip) {
		...
		int width = _srcRect.right - _srcRect.left;
		int height = _srcRect.bottom - _srcRect.top;
...
void Drawing::draw(LPDIRECT3DTEXTURE9 texture, RECT _srcRect, D3DXVECTOR3 _position, bool _isFlip, bool _isVerticalFlip, D3DXVECTOR2 _translate, D3DCOLOR _color)
{
	...
	if (_isFlip || _isVerticalFlip) {
		D3DXVECTOR2 scalePoint;
		D3DXVECTOR2 translation = D3DXVECTOR2(0, 0);
		D3DXVECTOR2 tempScale = D3DXVECTOR2(_isFlip == true ? -1.0f : 1.0f, _isVerticalFlip == true ? -1.0f : 1.0f);
