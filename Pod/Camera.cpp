...
Camera::Camera(float _x, float _y, float _vx, float _vy, float _dt, float _limitX, float _limitY) : Component(_x, _y, _vx, _vy, _dt, _limitX, limitY)
{
	Component::Component(_x, _y, _vx, _vy, _dt, _limitX, limitY);
...
{
	if (instance == nullptr) {
		instance = new Camera(0, 0, 0, 0, 0, 0, 0);
	}
	return instance;
}
...
void Camera::load()
{
	...
	if (v.size() == 7) {
		...
		this->vy = stoi(v[3]);
		this->dt = stoi(v[4]);
		this->limitX = stoi(v[5]);
		this->limitY = stoi(v[6]);
	}
}
